...
<table class="table category table-striped">
  ...
  <tr>
    ...
    <?php if($this->params->get('list_show_author')) : ?>
      <th width="25%">
	<?php echo JText::_('COM_SONGBOOK_HEADING_CREATE_BY'); ?>
      </th>
    <?php endif; ?>
    <?php if($this->params->get('list_show_date')) : ?>
      <th width="15%" id="categorylist_header_date">
	<?php $date = $this->params->get('order_date'); ?>
	<?php echo JText::_('COM_SONGBOOK_'.strtoupper($date).'_DATE'); ?>
      </th>
    <?php endif; ?>
  </tr>
  </thead>
  ...
  <tbody>
	...
	<?php  //Build the link to the login page for the user to login or register.
		...
		$returnURL = JRoute::_(SongbookHelperRoute::getSongRoute($item->slug, $item->tagid, $item->language));
		$link = new JUri($link1);
		$link->setVar('return', base64_encode($returnURL));
	      endif; ?>

	<?php if($item->params->get('access-view')) : //Set the link to the song page.
	      $link = JRoute::_(SongbookHelperRoute::getSongRoute($item->slug, $item->tagid, $item->language));
	  endif; ?>
	  ...
	  <?php if($this->params->get('list_show_date')) : ?>
		...
		<?php echo JText::_('COM_SONGBOOK_UNMODIFIED'); ?>
	      <?php else : ?>
