...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
			...
			<?php if ($this->params->get('filter_field')) : ?>
				<div class="btn-group">
					<label class="filter-search-lbl element-invisible" for="filter-search">
						<?php echo JText::_('COM_SONGBOOK_TITLE_FILTER_LABEL') . '&#160;'; ?>
					</label>
					<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="inputbox" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_SONGBOOK_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_SONGBOOK_TITLE_FILTER_LABEL'); ?>" />
	...
	<?php if ($this->items == false || $n === 0) : ?>
		<p><?php echo JText::_('COM_SONGBOOK_NO_TAGS'); ?></p>
	<?php else : ?>
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<?php if ((!empty($item->access)) && in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
				<li class="cat-list-row<?php echo $i % 2; ?>">
					<h3>
						<a href="<?php echo JRoute::_(SongbookHelperRoute::getTagRoute($item->id.':'.$item->alias, $item->language)); ?>">
