...
?>
  ...
  <?php if($params->get('filter_ordering')) : ?>
	...
	<?php echo JText::_('COM_SONGBOOK_SORTING_FILTER_LABEL').'&#160;'; ?>
      </label>
      <select name="filter-ordering" id="filter-ordering" onchange="songbook.submitForm();">
	<option value=""><?php JText::_('JGLOBAL_SORT_BY'); ?></option>
	<?php $selected = ($filterOrdering === 'order') ? 'selected="selected"' : ''; ?>
	<option value="order" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_ORDERING_ASC'); ?></option>
	<?php $selected = ($filterOrdering === 'rorder') ? 'selected="selected"' : ''; ?>
	<option value="rorder" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_ORDERING_DESC'); ?></option>
	<?php $selected = ($filterOrdering === 'alpha') ? 'selected="selected"' : ''; ?>
	<option value="alpha" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_TITLE_ASC'); ?></option>
	<?php $selected = ($filterOrdering === 'ralpha') ? 'selected="selected"' : ''; ?>
	<option value="ralpha" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_TITLE_DESC'); ?></option>
	<?php if($optionDate) : ?>
	  <?php $selected = ($filterOrdering === 'date') ? 'selected="selected"' : ''; ?>
	  <option value="date" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_DATE_ASC'); ?></option>
	  <?php $selected = ($filterOrdering === 'rdate') ? 'selected="selected"' : ''; ?>
	  <option value="rdate" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_DATE_DESC'); ?></option>
	<?php endif; ?>
	<?php if($optionAuthor) : ?>
	  <?php $selected = ($filterOrdering === 'author') ? 'selected="selected"' : ''; ?>
	  <option value="author" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_AUTHOR_ASC'); ?></option>
	  <?php $selected = ($filterOrdering === 'rauthor') ? 'selected="selected"' : ''; ?>
	  <option value="rauthor" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_AUTHOR_DESC'); ?></option>
	<?php endif; ?>
	<?php if($optionDownloads) : ?>
	  <?php $selected = ($filterOrdering === 'hits') ? 'selected="selected"' : ''; ?>
	  <option value="downloads" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_HITS_ASC'); ?></option>
	  <?php $selected = ($filterOrdering === 'rhits') ? 'selected="selected"' : ''; ?>
	  <option value="rdownloads" <?php echo $selected; ?>><?php echo JText::_('COM_SONGBOOK_ORDERING_HITS_DESC'); ?></option>
	<?php endif; ?>
      </select>
    </div>
  <?php endif; ?>
