 ...
 * List of Tags field.
 *
 * Note: Original file: libraries/src/Form/Field/TagField.php
 *
 * @since  3.1
 */
class JFormFieldTags extends JFormFieldList
{
  ...
  protected function getOptions()
  {
    ...
    $db    = JFactory::getDbo();
    //Override: Adds the language attribute to the query.
    $query = $db->getQuery(true)
	    ->select('DISTINCT a.id AS value, a.path, a.title AS text, a.level, a.published, a.lft, a.language')
	    ->from('#__tags AS a')
    ...
    if ($this->isNested())
    {
      ...
      //Override: Displays the language tag.
      foreach($options as $key => $option) {
	if($option->language !== '*') {
	  $options[$key]->text = $option->text.' ('.$option->language.')';
	}
      }
    }
    else
