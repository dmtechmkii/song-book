...
class com_songbookInstallerScript
{
  /**
  ...
  function preflight($type, $parent) 
  {
    ...
    echo '<p>'.JText::_('COM_SONGBOOK_INSTALLING_COMPONENT_VERSION').$this->release;
    echo '<br />'.JText::_('COM_SONGBOOK_CURRENT_JOOMLA_VERSION').$jversion->getShortVersion().'</p>';
  ...
  function postflight($type, $parent) 
  {
    if($type == 'install') {
      ...
      $form = new JForm('songbook_config');
      //Note: The third parameter must be set or the xml file won't be loaded.
      $form->loadFile(JPATH_ROOT.'/administrator/components/com_songbook/config.xml', true, '/config');
      $JsonValues = '';
      ...
      $query->where('element='.$db->Quote('com_songbook').' AND type='.$db->Quote('component'));
      $db->setQuery($query);
      $db->query();
